...
let s_danmu = function () {
    ...
    panelBtn = Player.util.createDom('xg-panel', `<xg-panel-icon class="xgplayer-panel-icon">
                                              ...
                                              <xg-panel-slider class="xgplayer-panel-slider">
                                                ...
                                                <xg-transparency class="xgplayer-transparency">
                                                  ...
                                                  <span class="xgplayer-transparency-text"></span>
                                                </xg-transparency>
                                                ...
                                                <xg-danmuspeed class="xgplayer-danmuspeed">
                                                  <span>弹幕速度</span>
                                                  <input class="xgplayer-danmuspeed-line xgplayer-danmuspeed-color xgplayer-danmuspeed-bar xgplayer-danmuspeed-gradient" type="range" min="50" max="150" value="100">
                                                  <span class="xgplayer-danmuspeed-text"></span>
                                                </xg-danmuspeed>
                                                <xg-danmufont class="xgplayer-danmufont">
                                                  <span>字体大小</span>
                                                  <input class="xgplayer-danmufont-line xgplayer-danmufont-color xgplayer-danmufont-bar xgplayer-danmufont-gradient" type="range" min="20" max="30" value="25">
                                                  <span class="xgplayer-danmufont-text"></span>
                                                </xg-danmufont>
  ...
  player.once('complete', () => {
    ...
    let transparencyText = panelBtn.querySelector('.xgplayer-transparency-text')
    let transparencyValue = 50
    transparencyText.innerText = '50%';
    ...
    transparency.addEventListener('input', function (e) {
      ...
      transparencyText.innerText = Math.ceil(e.target.value) + '%';
      transparencyGradient.style.background = 'linear-gradient(to right, #f85959 0%, #f85959 ' + transparencyValue + '%, #aaa ' + transparencyValue + '%, #aaa)'
    ...
    let danmuspeed = panelBtn.querySelector('.xgplayer-danmuspeed-line')
    let danmuspeedGradient = panelBtn.querySelector('.xgplayer-danmuspeed-gradient')
    let danmuspeedText = panelBtn.querySelector('.xgplayer-danmuspeed-text')
    let danmuspeedValue = 100;
    danmuspeedText.innerText = '中';
    let danmuspeedDegree = Math.ceil(100 - (150 - danmuspeedValue));
    danmuspeedGradient.style.background = 'linear-gradient(to right, #f85959 0%, #f85959 ' +  danmuspeedDegree + '%, #aaa ' + danmuspeedDegree + '%, #aaa)'
    danmuspeed.addEventListener('input', function (e) {
      e.preventDefault()
      e.stopPropagation()
      danmuspeedValue = e.target.value
      if(danmuspeedValue == 150) {
        danmuspeedText.innerText = '特快';
      } else if(danmuspeedValue < 150 && danmuspeedValue > 100) {
        danmuspeedText.innerText = '快';
      } else if(danmuspeedValue == 100) {
        danmuspeedText.innerText = '中';
      } else if(danmuspeedValue < 100 && danmuspeedValue > 50) {
        danmuspeedText.innerText = '慢';
      } else if(danmuspeedValue == 50) {
        danmuspeedText.innerText = '特慢';
      }
      danmuspeedDegree = Math.ceil(100 - (150 - danmuspeedValue));
      danmuspeedGradient.style.background = 'linear-gradient(to right, #f85959 0%, #f85959 ' + danmuspeedDegree + '%, #aaa ' + danmuspeedDegree + '%, #aaa)'
      danmuConfig.comments.forEach(item => {
    ...
    let danmufontGradient = panelBtn.querySelector('.xgplayer-danmufont-gradient')
    let danmufontText = panelBtn.querySelector('.xgplayer-danmufont-text')
    let danmufontValue = 25;
    danmufontText.innerText = '中';
    let danmufontDegree = Math.ceil(100 - ((30 - danmufontValue) * 10));
    danmufontGradient.style.background = 'linear-gradient(to right, #f85959 0%, #f85959 ' +  danmufontDegree + '%, #aaa ' + danmufontDegree + '%, #aaa)'
    danmufont.addEventListener('input', function (e) {
      e.preventDefault()
      e.stopPropagation()
      danmufontValue = e.target.value;
      if(danmufontValue == 30) {
        danmufontText.innerText = '特大';
      } else if(danmufontValue < 30 && danmufontValue > 25) {
        danmufontText.innerText = '大';
      } else if(danmufontValue == 25) {
        danmufontText.innerText = '中';
      } else if(danmufontValue < 25 && danmufontValue > 20) {
        danmufontText.innerText = '小';
      } else if(danmufontValue == 20) {
        danmufontText.innerText = '特小';
      }
      danmufontDegree = Math.ceil(100 - ((30 - danmufontValue) * 10));
      danmufontGradient.style.background = 'linear-gradient(to right, #f85959 0%, #f85959 ' + danmufontDegree + '%, #aaa ' + danmufontDegree + '%, #aaa)'
      danmuConfig.comments.forEach(item => {
